...
export async function GET(request) {
  ...
  if (!checkPermission(authResult.user, 'invoices', 'read')) {
    return Response.json(
      { error: 'ليس لديك صلاحية لعرض الصادرات' },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    const invoicesQuery = `
      ...
      FROM invoices_out i
      LEFT JOIN partners p ON i.customer_id = p.id
    ...
    console.error('Get invoices-out error:', error);
    return Response.json(
      { error: 'خطأ في جلب بيانات الصادرات' },
      { status: 500 }
    );
  }
}
...
export async function POST(request) {
  ...
  if (!checkPermission(authResult.user, 'invoices', 'create')) {
    return Response.json(
      { error: 'ليس لديك صلاحية لإنشاء صادرات جديدة' },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    const result = await sql`
      INSERT INTO invoices_out (
        store_id, invoice_number, customer_id, amount, currency, description,
        category, invoice_date, due_date, tax_amount, discount_amount,
        attachments, notes, created_by
      ) VALUES (
        ${user.storeId}, ${invoice_number}, ${customer_id}, ${amount}, ${currency},
    ...
    await logActivity(user, 'create', 'invoice_out', invoice.id, null, invoice);

    return Response.json({
      success: true,
      data: invoice,
      message: 'تم إنشاء الصادر بنجاح'
    });

  } catch (error) {
    console.error('Create invoice-out error:', error);
    return Response.json(
      { error: 'خطأ في إنشاء الصادر' },
      { status: 500 }
    );
  }
}
...
export async function PUT(request) {
  ...
  if (!checkPermission(authResult.user, 'invoices', 'update')) {
    return Response.json(
      { error: 'ليس لديك صلاحية لتعديل الصادرات' },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    const { 
      amount, currency, description, invoice_date, customer_id, category,
      due_date, tax_amount, discount_amount, payment_status, paid_amount,
      attachments, notes, invoice_number
    } = data;

    const result = await sql`
      UPDATE invoices_out SET
    ...
    await logActivity(user, 'update', 'invoice_out', invoiceId, oldInvoice, updatedInvoice);

    return Response.json({
      success: true,
      data: updatedInvoice,
      message: 'تم تحديث الصادر بنجاح'
    });

  } catch (error) {
    console.error('Update invoice-out error:', error);
    return Response.json(
      { error: 'خطأ في تحديث الصادر' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request) {
  ...
  if (authResult.user.role !== 'store_owner') {
    return Response.json(
      { error: 'فقط مدير المتجر يمكنه حذف الصادرات' },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    await logActivity(user, 'delete', 'invoice_out', invoiceId, existingInvoice[0], null);

    return Response.json({
      success: true,
      message: 'تم حذف الصادر بنجاح'
    });

  } catch (error) {
    console.error('Delete invoice-out error:', error);
    return Response.json(
      { error: 'خطأ في حذف الصادر' },
      { status: 500 }
    );
  }
}
